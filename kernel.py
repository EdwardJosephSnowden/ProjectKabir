...
def hello(parameter):
    greetings = ['namaste','howdy','hello','good evening', 'good morning','good afternoon']
    for x in parameter:
        for y in greetings:
            if x == y:
                speak(f"{y} Sir, I'm your assistant KABIR please tell me how may I help you")
                break
        break
...
def greet():
    ...
    speak(f"\n\nGood {tm}! Sir, I'm your assistant KABIR please tell me how may I help you")
...
def use_chatgpt():
    openai.api_key = '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'
    speak("What do you want to ask to ChatGPT")
    prompt = get_speech_input()

    response = openai.Completion.create(engine="gpt-3.5-turbo-0301", prompt=prompt, temperature=0.7, max_tokens=150, n=1)
...
def news():
    ...
    for keyword in ["technology", "science", "business", "entertainment", "health", "sports", "politics", "world news", "environment", "education"]:
        if keyword in topic:
            ...
            break
    headlines(topic)
    speak("do you want summary on any news \n if yes tell me the headline number \n else type exit")
...
def main():
    ...
    while True:
        choice = get_speech_input()
        if choice:
            if any(keyword in choice for keyword in ['namaste','howdy','hello','good evening', 'good morning','good afternoon']):
                c = choice.split()
                hello(c)
                ...
                search_wikipedia()
            elif any(keyword in choice for keyword in ['time', 'date', 'month', 'day']):
                ...
                use_chatgpt()
            elif any(keyword in choice for keyword in ['bye kabir', 'end program', 'exit program','goodbye kabir','exit']):
                exit()
            elif any(keyword in choice for keyword in ['to do','todo','to-do']):
                todo(choice)
            elif any(keyword in choice for keyword in ['current affair','update','news']):
                news()
            else:
